...
from soundtrack_app.config import RECENT_HISTORY_LIMIT, database_path
...
from soundtrack_app.services.library_scanner import LibraryScanner, ScanResult
...
from soundtrack_app.ui.dialogs.campaign_manager_dialog import CampaignManagerDialog
from soundtrack_app.ui.dialogs.duplicates_dialog import DuplicatesDialog
from soundtrack_app.ui.dialogs.export_dialog import ExportDialog
...
from soundtrack_app.ui.widgets.soundtrack_panel import SoundtrackPanel
...
class MainWindow(QWidget):
    """Widget do módulo Soundtrack: usável como janela central standalone
    (veja ``main.py``) ou embutido como página do RPG Audio Studio."""

    def __init__(self, db_path: Path | None = None, embed_player_bar: bool = True):
        super().__init__()
        self._embed_player_bar = embed_player_bar
    ...
    def _build_ui(self) -> None:
        ...
        # Quando embutido no RPG Audio Studio, a barra de player vira global
        # (mesma player_service, só que instanciada pelo shell) — ver item 17.
        self.player_bar: PlayerBar | None = None
        if self._embed_player_bar:
    ...
    def _install_shortcuts(self) -> None:
        # Contexto restrito ao próprio widget (e filhos): dentro do RPG Audio
        # Studio, várias páginas/módulos convivem na mesma janela e cada um
        # instala seus próprios atalhos — sem isso, um atalho como "Espaço"
        # dispararia mesmo com o módulo Soundtrack escondido em outra aba.
        for sequence, slot in (
            (Qt.Key.Key_Space, self._shortcut_play_pause),
            (Qt.Key.Key_Return, self._shortcut_add_to_soundtrack),
            (Qt.Key.Key_Enter, self._shortcut_add_to_soundtrack),
            (Qt.Key.Key_F, self._shortcut_toggle_favorite),
            (Qt.Key.Key_Down, lambda: self._shortcut_navigate(1)),
            (Qt.Key.Key_Up, lambda: self._shortcut_navigate(-1)),
        ):
            shortcut = QShortcut(QKeySequence(sequence), self, activated=slot)
            shortcut.setContext(Qt.ShortcutContext.WidgetWithChildrenShortcut)

    def set_module_active(self, active: bool) -> None:
        """Chamado pelo shell do Studio ao trocar de página (item 32: evita
        atalhos/ações do módulo dispararem quando ele não está em primeiro
        plano)."""
    ...
    def _shortcuts_enabled(self) -> bool:
        return self._is_active and not self._typing_in_text_field()
    ...
    def open_triage(self) -> None:
        self._on_open_triage()

    def create_new_soundtrack(self) -> None:
        self._on_new_soundtrack()
    ...
    def scan_folder(self, folder: Path) -> None:
        """Escaneia ``folder`` e passa a usá-la como pasta da biblioteca —
        usado pela integração Downloader → Biblioteca (item 15) para
        "Adicionar à Biblioteca" quando a pasta baixada é diferente da
