...
def test_default_conflict_combo_selection_without_initial_policy(qt_core_app):
    dialog = ExportDialog("Trilha", 3)
    assert dialog.conflict_combo.currentData() == "rename"


def test_initial_conflict_policy_preselects_the_combo(qt_core_app):
    dialog = ExportDialog("Trilha", 3, initial_conflict_policy="overwrite")
    assert dialog.conflict_combo.currentData() == "overwrite"


def test_initial_conflict_policy_skip(qt_core_app):
    dialog = ExportDialog("Trilha", 3, initial_conflict_policy="skip")
    assert dialog.conflict_combo.currentData() == "skip"


def test_unknown_initial_conflict_policy_is_ignored_gracefully(qt_core_app):
    dialog = ExportDialog("Trilha", 3, initial_conflict_policy="valor-invalido")
    # não trava, e mantém o primeiro item (comportamento padrão do QComboBox)
    assert dialog.conflict_combo.currentData() == "rename"
